...
if st.button("🔎 Predict"):
    ...
    try:
        # Ensure both the repo root AND src/ are on sys.path before loading
        # the joblib file.  joblib unpickling resolves class names at import
        # time, so Custom_Classes must be findable both as a top-level module
        # AND via the 'src' namespace that was baked into the pickle.
        ...
        # X_train.csv holds the post-pipeline columns used as SHAP background
        X_train_bg = pd.read_csv(X_TRAIN_PATH)
        bg_cols    = X_train_bg.columns.tolist()

        # Build a mini transform-only pipeline (skip sampler + model)
        ...
        # Align columns with the background dataset
        ...
        # RF classifiers return shape (1, n_features, 2) — slice to class 1 (Fraud)
