...
# ── NOTE: statsmodels, gensim and spacy are imported lazily inside their
#    respective classes so that the fraud-detection pipeline (which never
#    uses PairFeatureEngineer, Word2Vec or SpaCy) can be unpickled on
#    SageMaker without requiring those heavy / incompatible packages.
...
class FeatureSelector(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        # 2. High Cardinality Filter (only categorical columns)
        ...
        # 3. Correlation Filter (only numeric columns)
...
class PairFeatureEngineer(BaseEstimator, TransformerMixin):
    ...
    def _compute_rolling_regression(self, df):
        # Lazy import — statsmodels is only needed for PairFeatureEngineer
...
class Word2VecTransformer(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        # Lazy import — only needed when this transformer is actually used
